...
it('adds the cleanup queue, tag index and scan audit tables when upgrading V15 and preserves old business data and damaged diagnostic state', () => {
  ...
  try {
    const schemaBefore = db.prepare("SELECT type, name, tbl_name, sql FROM sqlite_master WHERE name NOT LIKE 'sqlite_%' AND name NOT GLOB 'library_scan_audit_*' AND name <> 'idx_video_tag_tag_id' AND name <> 'media_library_configs' ORDER BY name").all()
    ...
    const schemaAfter = db.prepare("SELECT type, name, tbl_name, sql FROM sqlite_master WHERE name NOT LIKE 'sqlite_%' AND name NOT GLOB 'library_scan_audit_*' AND name <> 'agent_resource_cleanup' AND name <> 'idx_video_tag_tag_id' AND name <> 'media_library_configs' ORDER BY name").all()
